...
環境で実行された後に Deploy インスタンスのネットワーク詳細を変更することはサポートされていません。詳細については、ナレッジベースの記事link:https://kb.netapp.com/onprem/ontap/ONTAP_Select/Modifying_DNS_configuration_of_ONTAP_Deploy_instance["ONTAP DeployインスタンスのDNS構成の変更"^]を参照してください。
...
はい。ONTAP Select DeployはKVMまたはESXiのいずれかにインストールでき、どちらのインストールでもいずれかのハイパーバイザー上にONTAP Selectクラスタを作成できます。
...
ONTAP Selectノードをリカバリする必要がある場合（ONTAP Select VMまたはホストが完全に失われた場合やシングルノード クラスタが誤って削除された場合など）は、NetAppテクニカルサポートにお問い合わせください。詳細については、ナレッジベースの記事link:https://kb.netapp.com/on-prem/ontap/ontap-select/Select-KBs/ONTAP_Select_node_VM_recovery_in_case_of_disaster["災害時のONTAP SelectノードVMリカバリ"^]を参照してください。
