...
. ページ右上の図のアイコンを選択し、次に*パスワードの変更*を選択します。
. 指示に従って現在のパスワードと新しいパスワードを入力し、* Submit * を選択します。
...
. 管理者アカウントを使用して、 Deploy ユーティリティの Web ユーザインターフェイスにサインインします。
. ページ上部の*[管理]*タブを選択します。
. *Management Servers* > *Add vCenter* を選択します。
. 以下の情報を入力し、*追加*を選択します。
+
[cols="30,70"]
|===
...
|===
. オプションで、新しい管理サーバーを追加した後にimage:icon_kebab.gif["オプション（ Options ）"]を選択し、次のいずれかを選択します。
+
** クレデンシャルを更新
** クレデンシャルを検証する
** 管理サーバを削除します
