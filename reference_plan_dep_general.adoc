...
* *ホスト*：VMware ESXi にONTAP Select ノードを直接デプロイする際に、ハイパーバイザーホストを認証するために使用されます
* *vCenter*: ホストが VMware vCenter によって管理されている場合に、ONTAP Select ノードを ESXi にデプロイする一環として vCenter サーバーを認証するために使用されます
