...
外部キー管理（KMIP）のサポート:: ONTAP Select 9.19.1以降、ONTAP Selectは鍵管理相互運用プロトコル（KMIP）を使用した外部鍵管理をサポートします。ONTAP Select 9.19.1クラスタの導入後、または既存のクラスタをONTAP Select 9.19.1にアップグレードした後に、外部鍵管理を有効にすることができます。外部鍵管理は、ONTAP Select 9.19.1でサポートされている唯一の鍵管理です。詳細については、link:reference_lic_ontap_features.html#licensed-ontap-features["デフォルトでライセンスされるONTAP機能"]を参照してください。
+
--
ONTAP Select は、クラウドキーマネージャーや ONTAP Onboard Key Manager をサポートしていません。
