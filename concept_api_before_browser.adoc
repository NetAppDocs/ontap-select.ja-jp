...
---
= ブラウザを使用した ONTAP Select Deploy API へのアクセスについて
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 値の例 API 呼び出しで _値の例_ を選択すると、その呼び出しの典型的な JSON 構造が表示されます。必要に応じて例を修正し、リクエストの入力として使用できます。
* Model _Model_を選択すると、JSONパラメータの完全なリストが表示され、各パラメータの説明が表示されます。
