...
に従って、link:reference_chk_deploy_req_info.html["Deploy ユーティリティのインストールに必要な情報"]およびlink:reference_chk_select_req_info.html["ONTAP Select のインストールに必要な情報"]のトピックで詳細をlink:kvm-host-configuration-and-preparation-checklist.html["ホスト準備チェックリスト"]確認してください。
