...
---
= ONTAP Select Deploy APIへのアクセスについて
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: ONTAP Select Deploy に付属の REST API にはバージョン番号が割り当てられます。API のバージョン番号は、Deploy のリリース番号とは無関係です。ONTAP Select 9.19.1 Deploy 管理ユーティリティには、バージョン 3 の REST API が含まれています。
